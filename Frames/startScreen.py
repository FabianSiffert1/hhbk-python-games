...
class startScreen:
    ...
    def __init__(self,vFrame):
        ...
        loginButton = Button(frame, text="Login", command= lambda: vFrame.openScreen("login"))
        loginButton.grid(column=0,row=2,padx=5,pady=2.5)

        settingsButton = Button(frame, text="Settings", command= lambda: vFrame.openScreen("settings"))
        settingsButton.grid(column=0,row=3,padx=5,pady=2.5)

        quitButton = Button(frame, text="Quit", command= vFrame.mainWindow.destroy)
        quitButton.grid(column=0,row=4,padx=5,pady=2.5)
